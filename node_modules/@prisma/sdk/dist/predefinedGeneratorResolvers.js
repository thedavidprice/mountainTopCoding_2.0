...
const load = require;
async function getPrismaClientDir(baseDir) {
    const handler = (base, item) => {
        const itemPath = path_1.default.join(base, item);
    ...
    return (await find_1.findUpAsync(baseDir, ['package.json'], ['f'], ['d', 'l'], 1, handler))[0];
}
    ...
    try {
        ...
        if (output.stdout) {
            ...
            if (semverLt(currentVersion, minVersion)) {
                throw new Error(`Your ${chalk_1.default.bold('typescript')} version is ${currentVersion}, which is outdated. Please update it to ${chalk_1.default.bold(minVersion)} or ${chalk_1.default.bold('newer')} in order to use Prisma Client.`);
            }
        }
    }
    catch (e) {

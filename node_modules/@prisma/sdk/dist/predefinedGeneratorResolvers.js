...
const debug = debug_1.default('prisma:generator');
/**
...
function findPrismaClientDir(baseDir) {
    const CLIDir = resolve_pkg_1.default('prisma', { cwd: baseDir });
    const clientDir = resolve_pkg_1.default('@prisma/client', { cwd: baseDir });
    ...
    const relDir = path_1.default.relative(CLIDir, clientDir).split(path_1.default.sep);
...
exports.predefinedGeneratorResolvers = {
  ...
  "license": "ISC"
        ...
        `;
            ...
            prismaClientDir = findPrismaClientDir(path_1.default.join('.', baseDir));
            if (!prismaClientDir) {
...
function checkTypeScriptVersion() {
    ...
    try {
        const typescriptPath = resolve_pkg_1.default('typescript', { cwd: process.cwd() });
        const typescriptPkg = typescriptPath && path_1.default.join(typescriptPath, 'package.json');
        if (typescriptPkg && fs_1.default.existsSync(typescriptPkg)) {
            const pjson = require(typescriptPkg);
            const currentVersion = pjson.version;
            if (semverLt(currentVersion, minVersion)) {
                _1.logger.warn(`Prisma detected that your ${chalk_1.default.bold('TypeScript')} version ${currentVersion} is outdated. If you want to use Prisma Client with TypeScript please update it to version ${chalk_1.default.bold(minVersion)} or ${chalk_1.default.bold('newer')}`);
            }
        }
    }
    catch (e) {

...
const mongoFeatureFlagMissingMessage_1 = require("./utils/mongoFeatureFlagMissingMessage");
const parseEnvValue_1 = require("./utils/parseEnvValue");
...
version, cliVersion, printDownloadProgress, baseDir = path_1.default.dirname(schemaPath), overrideGenerators, skipDownload, binaryPathsOverride, }) {
    ...
    const queryEngineBinaryType = process.env.PRISMA_FORCE_NAPI === 'true'
        ? fetch_engine_1.BinaryType.libqueryEngineNapi
        : fetch_engine_1.BinaryType.queryEngine;
    const queryEngineType = binaryTypeToEngineType(queryEngineBinaryType);
    let prismaPath = binaryPathsOverride === null || binaryPathsOverride === void 0 ? void 0 : binaryPathsOverride[queryEngineType];
    ...
    const config = await _1.getConfig({
        datamodel,
    ...
    const previewFeatures = mapPreviewFeatures_1.mapPreviewFeatures(extractPreviewFeatures_1.extractPreviewFeatures(config));
    const dmmf = await _1.getDMMF({
        datamodel,
        datamodelPath: schemaPath,
        prismaPath,
        previewFeatures,
    ...
    if (config.datasources.some((d) => d.provider.includes('mongoDb')) &&
        ...
        throw new Error(mongoFeatureFlagMissingMessage_1.mongoFeatureFlagMissingMessage);
    }
    ...
    try {
        ...
        const generators = await p_map_1.default(generatorConfigs, async (generator, index) => {
            let generatorPath = parseEnvValue_1.parseEnvValue(generator.provider);
            let paths;
            // as of now mostly used by studio
            const providerValue = parseEnvValue_1.parseEnvValue(generator.provider);
            if (aliases && aliases[providerValue]) {
                generatorPath = aliases[providerValue].generatorPath;
                paths = aliases[providerValue];
            }
            else if (predefinedGeneratorResolvers_1.predefinedGeneratorResolvers[providerValue]) {
                paths = await predefinedGeneratorResolvers_1.predefinedGeneratorResolvers[providerValue](baseDir, cliVersion);
            ...
            if (generator.output) {
                generator.output = {
                    value: path_1.default.resolve(baseDir, parseEnvValue_1.parseEnvValue(generator.output)),
                    fromEnvVar: null,
                };
...
The generator needs to either define the \`defaultOutput\` path in the manifest or you need to define \`output\` in the datamodel.prisma file.`);
                ...
                generator.output = {
                    value: await resolveOutput_1.resolveOutput({
        ...
        const generatorProviders = generatorConfigs.map((g) => parseEnvValue_1.parseEnvValue(g.provider));
...
`);
        ...
        for (const g of generators) {
            if (((_d = g.manifest) === null || _d === void 0 ? void 0 : _d.requiresEngines) &&
                ...
                const generatorBinaryTargets = (_h = (_g = g.options) === null || _g === void 0 ? void 0 : _g.generator) === null || _h === void 0 ? void 0 : _h.binaryTargets;
                if (generatorBinaryTargets && generatorBinaryTargets.length > 0) {
                    ...
                    if (binaryTarget0.fromEnvVar !== null) {
                        const parsedBinaryTargetsEnvValue = parseEnvValue_1.parseBinaryTargetsEnvValue(binaryTarget0);
        ...
        for (const generator of generators) {
            if (generator.manifest && generator.manifest.requiresEngines) {
                ...
                if (engineVersion !== version &&
                    ...
                    ((_j = generatorBinaryPaths[queryEngineType]) === null || _j === void 0 ? void 0 : _j[platform])) {
                    const customDmmf = await _1.getDMMF({
                        datamodel,
                        datamodelPath: schemaPath,
                        prismaPath: (_k = generatorBinaryPaths[queryEngineType]) === null || _k === void 0 ? void 0 : _k[platform],
                        previewFeatures,
                    });
...
async function getBinaryPathsByVersion({ neededVersions, platform, version, printDownloadProgress, skipDownload, binaryPathsOverride, }) {
    ...
    for (const currentVersion in neededVersions) {
        ...
        if (process.env.NETLIFY &&
            !neededVersion.binaryTargets.find((object) => object.value === 'rhel-openssl-1.0.x')) {
...
async function validateGenerators(generators) {
    const platform = await get_platform_1.getPlatform();
    for (const generator of generators) {
        if (parseEnvValue_1.parseEnvValue(generator.provider) === 'photonjs') {
...
Please use the PRISMA_QUERY_ENGINE_BINARY env var instead to pin the binary target.`);
        ...
        if (generator.binaryTargets) {
            ...
            const resolvedBinaryTargets = binaryTargets
                .flatMap((object) => parseEnvValue_1.parseBinaryTargetsEnvValue(object))
            ...
            for (const resolvedBinaryTarget of resolvedBinaryTargets) {
                if (oldToNewBinaryTargetsMapping[resolvedBinaryTarget]) {
                    throw new Error(`Binary target ${chalk_1.default.red.bold(resolvedBinaryTarget)} is deprecated. Please use ${chalk_1.default.green.bold(oldToNewBinaryTargetsMapping[resolvedBinaryTarget])} instead.`);
                }
                if (!exports.knownBinaryTargets.includes(resolvedBinaryTarget)) {
                    throw new Error(`Unknown binary target ${chalk_1.default.red(resolvedBinaryTarget)} in generator ${chalk_1.default.bold(generator.name)}.
Possible binaryTargets: ${chalk_1.default.greenBright(exports.knownBinaryTargets.join(', '))}`);
            ...
            if (!resolvedBinaryTargets.includes(platform)) {
                const originalBinaryTargetsConfig = engine_core_1.getOriginalBinaryTargetsValue(generator.binaryTargets);
                if (generator) {
                    console.log(`${chalk_1.default.yellow('Warning:')} Your current platform \`${chalk_1.default.bold(platform)}\` is not included in your generator's \`binaryTargets\` configuration ${JSON.stringify(originalBinaryTargetsConfig)}.
    ...
    Read more about deploying Prisma Client: ${chalk_1.default.underline('https://github.com/prisma/prisma/blob/master/docs/core/generators/prisma-client-js.md')}`)}\n`);
                    ...
                    console.log(`${chalk_1.default.yellow('Warning')} The binaryTargets ${JSON.stringify(originalBinaryTargetsConfig)} don't include your local platform ${platform}, which you can also point to with \`native\`.
...
function engineTypeToBinaryType(engineType) {
    if (engineType === 'introspectionEngine') {
        return fetch_engine_1.BinaryType.introspectionEngine;
    }
    if (engineType === 'migrationEngine') {
        return fetch_engine_1.BinaryType.migrationEngine;
    }
    if (engineType === 'queryEngine') {
        return fetch_engine_1.BinaryType.queryEngine;
    }
    if (engineType === 'libqueryEngineNapi') {
        return fetch_engine_1.BinaryType.libqueryEngineNapi;
    }
    if (engineType === 'prismaFmt') {
        return fetch_engine_1.BinaryType.prismaFmt;
    }
    throw new Error(`Could not convert engine type ${engineType}`);
}
function binaryTypeToEngineType(binaryType) {
    if (binaryType === fetch_engine_1.BinaryType.introspectionEngine) {
        return 'introspectionEngine';
    }
    if (binaryType === fetch_engine_1.BinaryType.migrationEngine) {
        return 'migrationEngine';
    }
    if (binaryType === fetch_engine_1.BinaryType.libqueryEngineNapi) {
        return 'libqueryEngineNapi';
    }
    if (binaryType === fetch_engine_1.BinaryType.queryEngine) {
        return 'queryEngine';
    }
    if (binaryType === fetch_engine_1.BinaryType.prismaFmt) {
        return 'prismaFmt';
    }

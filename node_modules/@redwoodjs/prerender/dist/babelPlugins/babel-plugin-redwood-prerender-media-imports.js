...
const BLANK_IMG_DATA_SRC = 'data:image/gif;base64,R0lGODlhAQABAIAAAP///wAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==';

function _default({
  types: t
}) {
  const manifestPath = (0, _path.join)((0, _internal.getPaths)().web.dist, 'manifest.json');
  ...
  return {
    ...
    visitor: {
      ImportDeclaration(p, state) {
        ...
        const ext = (0, _path.extname)(p.node.source.value);
        ...
        if (ext && (0, _includes.default)(_context = options.extensions).call(_context, ext)) {
          ...
          if (importConstName) {
            p.replaceWith(t.variableDeclaration('const', [t.variableDeclarator(t.identifier(importConstName), t.stringLiteral(processedFilePath || BLANK_IMG_DATA_SRC))]));
          }
        }
      }

...
const getConfigPath = (cwd = (() => {
  var _process$env$__REDWOO;

  return (_process$env$__REDWOO = process.env.__REDWOOD__CONFIG_PATH) !== null && _process$env$__REDWOO !== void 0 ? _process$env$__REDWOO : process.cwd();
})()) => {
...
const getPaths = (BASE_DIR = getBaseDir()) => {
  ...
  const globals = _path.default.join(BASE_DIR, '.redwood', 'globals');
...
const processPagesDir = (webPagesDir = getPaths().web.pages) => {
  const pagePaths = _glob.glob.sync('**/**/*Page.{js,jsx,ts,tsx}', {
    cwd: webPagesDir
  });

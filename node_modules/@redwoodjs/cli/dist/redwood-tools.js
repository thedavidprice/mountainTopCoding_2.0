...
const rwtLink = async yargs => {
  ...
  const projectPackagesPath = _path.default.join((0, _internal.getPaths)().base, 'redwood');
  ...
  await (0, _execa.default)('yarn build:link', ['--dest', projectPackagesPath, ...onlyParams], {
    shell: true,
    stdio: 'inherit',
    cleanup: true,
    cwd: frameworkPath
...
const rwtUnlink = () => {
  const linkedPackagesPath = _path.default.join((0, _internal.getPaths)().base, 'redwood');
...
const addRedwoodFolderToGitIgnore = () => {
  const gitIgnore = _fs.default.readFileSync(_path.default.join((0, _internal.getPaths)().base, '.gitignore'), 'utf-8');

  if ((0, _includes.default)(gitIgnore).call(gitIgnore, 'redwood/*')) {
    console.log('Redwood folder already in gitignore');
  } else {
    console.log('Adding `redwood/*` to .gitignore...');

    _fs.default.appendFileSync(_path.default.join((0, _internal.getPaths)().base, '.gitignore'), 'redwood/*');
  }
};
...
const updateProjectWithResolutions = (redwoodPackagesPath, remove) => {
  ...
  if (remove) {
    ...
    packages = (0, _filter.default)(packages).call(packages, workspaceFolder => workspaceFolder !== 'redwood/*');
  } else {
    resolutions = { ...resolutions,
      ...frameworkRepoResolutions
    };

    if (!(0, _includes.default)(packages).call(packages, 'redwood/*')) {
...
_yargs.default.command(['copy [RW_PATH]', 'cp'], 'Copy the Redwood Framework path to this project', {}, rwtCopy).command(['copy:watch [RW_PATH]', 'cpw'], 'Watch the Redwood Framework path for changes and copy them over to this project', {}, rwtCopyWatch).command({
  ...
  command: 'unlink',
  desc: 'Unlink your local verison of redwood, and use the one specified in package.json',
  handler: rwtUnlink

...
const builder = yargs => {
  yargs.usage('usage: $0 <side>').command({
    ...
    if (positionalArgs.includes('web') && !_fs.default.existsSync(_path.default.join((0, _internal.getPaths)().web.dist), 'index.html')) {
      console.error(_colors.default.error('\n Please run `yarn rw build web` before trying to serve web. \n'));
      process.exit(1);
    }

    if (positionalArgs.includes('api') && !_fs.default.existsSync(_path.default.join((0, _internal.getPaths)().api.dist))) {

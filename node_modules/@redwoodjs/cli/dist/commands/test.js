...
const handler = async ({
  ...
  const sides = filterParams.filter(filterString => (0, _structure.getProject)().sides.includes(filterString)); // All the other params, apart from sides

  const jestFilterArgs = [...filterParams.filter(filterString => !(0, _structure.getProject)().sides.includes(filterString))];
  const jestArgs = [...jestFilterArgs, ...forwardJestFlags, collectCoverage ? '--collectCoverage' : null, '--passWithNoTests', ...jestFilterArgs, sides.includes('api') && '--runInBand'].filter(flagOrValue => flagOrValue !== null); // Filter out nulls, not booleans because user may have passed a --something false flag
  ...
  if (!sides.length) {
    (0, _structure.getProject)().sides.forEach(side => sides.push(side));
  }

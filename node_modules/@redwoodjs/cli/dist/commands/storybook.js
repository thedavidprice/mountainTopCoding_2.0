...
const handler = ({
  ...
  (0, _execa.default)(`yarn ${build ? 'build' : 'start'}-storybook`, (0, _filter.default)(_context = ['--config-dir ../node_modules/@redwoodjs/core/config/storybook', !build && `--port ${port}`, !build && '--no-version-updates', !build && `--static-dir "${staticAssetsFolder}"`, build && `--output-dir "${_path.default.join((0, _internal.getPaths)().web.base, buildDirectory)}"`, !open && '--ci']).call(_context, Boolean), {
    stdio: 'inherit',
    shell: true,
    cwd
  });

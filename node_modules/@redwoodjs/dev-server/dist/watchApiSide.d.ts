import { NodeTargetPaths } from '@redwoodjs/internal';
export interface Functions {
    [path: string]: any;
}
/**
 * Purge the require cache and import them again.
 */
export declare const importFreshFunctions: (functionsPath: string) => Functions;
export declare const watchFunctions: ({ paths, onChange, onImport, onException, }: {
    paths: NodeTargetPaths;
    onChange: (event: string, path: string) => void;
    onImport: (functions: Functions) => void;
    onException: (e: Error) => void;
}) => void;
//# sourceMappingURL=watchApiSide.d.ts.map